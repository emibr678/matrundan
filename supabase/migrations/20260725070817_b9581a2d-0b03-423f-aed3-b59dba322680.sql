...
-- =========================================================================
-- 1. Lock down raw table SELECT for linking tables
-- =========================================================================

REVOKE SELECT ON public.group_places FROM authenticated, anon;
REVOKE SELECT ON public.visit_group_links FROM authenticated, anon;
REVOKE SELECT ON public.review_group_visibility FROM authenticated, anon;

-- Drop the read policies (moot without grant, but keep DB tidy)
DROP POLICY IF EXISTS "group_places members read" ON public.group_places;
DROP POLICY IF EXISTS "visit_group_links members read" ON public.visit_group_links;

-- Helper for places RLS: authenticated no longer sees group_places directly.
CREATE OR REPLACE FUNCTION public.can_see_place(_place_id uuid, _user_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.group_places gp
    JOIN public.memberships m
      ON m.group_id = gp.group_id AND m.user_id = _user_id AND m.status = 'active'
    WHERE gp.place_id = _place_id
  );
$$;
REVOKE ALL ON FUNCTION public.can_see_place(uuid,uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.can_see_place(uuid,uuid) TO authenticated;

DROP POLICY IF EXISTS "places readable via group_places" ON public.places;
CREATE POLICY "places readable via can_see_place" ON public.places
  FOR SELECT TO authenticated
  USING (public.can_see_place(id, auth.uid()));

-- =========================================================================
-- 2. Read RPC: add countsForProgression, expose own review always
-- =========================================================================
...
BEGIN
  ...
  vp_visible AS (
    ...
    WHERE EXISTS (SELECT 1 FROM public.memberships mm
                  WHERE mm.group_id = _group_id AND mm.user_id = vp.user_id AND mm.status='active')
  ),
  vp_external AS (
  ...
  reviews_scoped AS (
    -- Include: (a) any review flagged rating_visible=true whose author is active in the group,
    --         (b) the current user's own review row (always, so UI can toggle visibility).
    ...
    WHERE (
      (rgv.rating_visible = true
        AND EXISTS (SELECT 1 FROM public.memberships mm
                    WHERE mm.group_id = _group_id AND mm.user_id = r.user_id AND mm.status='active'))
      OR r.user_id = _uid
    )
  ),
  visits_json AS (
  ...
  rows AS (
    SELECT
      ...
      (SELECT count(*)::int FROM vp
        WHERE NOT EXISTS (SELECT 1 FROM public.memberships mm
                          WHERE mm.group_id = mg.id AND mm.user_id = vp.user_id AND mm.status='active')
      ) AS external_participant_count,
      (SELECT jsonb_agg(jsonb_build_object(
         'id', p.id,
         'name', COALESCE(NULLIF(trim(p.display_name),''),'Medlem'),
         'avatar', p.avatar_emoji, 'avatarImage', p.avatar_url
       ))
       FROM vp
       JOIN public.memberships mm
         ON mm.group_id = mg.id AND mm.user_id = vp.user_id AND mm.status='active'
       LEFT JOIN public.profiles p ON p.id = vp.user_id
      ) AS visible_participants,
      (SELECT count(*)::int
         FROM public.reviews r
         JOIN public.memberships mm
           ON mm.group_id = mg.id AND mm.user_id = r.user_id AND mm.status='active'
...
BEGIN
  ...
  -- Ensure the place is linked in the target group (origin: shared_visit)
  ...
  -- Link the visit to the target group as shared
  ...
  -- Create visibility rows for reviews whose author has an active membership
  -- in the target group. Rating visible by default; own comment per param;
  -- other authors' comments hidden by default.
  ...
  WHERE r.visit_id = _visit_id
    AND EXISTS (SELECT 1 FROM public.memberships mm
                WHERE mm.group_id = _target_group_id AND mm.user_id = r.user_id AND mm.status='active')
  ON CONFLICT (review_id, group_id) DO NOTHING;

  -- Neutral activity in target group (no cross-group naming leak)
...
-- =========================================================================
-- 5. remove_shared_visit_from_group
-- =========================================================================
...
BEGIN
  ...
  -- Atomic: strip visibility for this group + drop the link. Never touch canonical rows.
...
REVOKE ALL ON FUNCTION public.remove_shared_visit_from_group(uuid,uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.remove_shared_visit_from_group(uuid,uuid) TO authenticated;

-- =========================================================================
-- 6. set_review_group_visibility
-- =========================================================================

CREATE OR REPLACE FUNCTION public.set_review_group_visibility(
  _review_id uuid, _group_id uuid,
  _rating_visible boolean, _comment_visible boolean
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
...
BEGIN
  ...
  END IF;
  SELECT r.id, r.user_id, r.visit_id INTO _r
  FROM public.reviews r WHERE r.id = _review_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Betyget finns inte'; END IF;
  IF _r.user_id <> _uid THEN
    RAISE EXCEPTION 'Endast recensionens författare kan ändra synligheten';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.visit_group_links
                 WHERE visit_id = _r.visit_id AND group_id = _group_id) THEN
    RAISE EXCEPTION 'Besöket är inte kopplat till gruppen';
  END IF;
  IF _comment_visible AND NOT _rating_visible THEN
    RAISE EXCEPTION 'Kommentaren kan bara vara synlig när betyget också är synligt';
  END IF;

  INSERT INTO public.review_group_visibility (review_id, group_id, rating_visible, comment_visible)
  VALUES (_review_id, _group_id, _rating_visible, _comment_visible)
  ON CONFLICT (review_id, group_id) DO UPDATE
    SET rating_visible = EXCLUDED.rating_visible,
        comment_visible = EXCLUDED.comment_visible,
        updated_at = now();
END $$;

REVOKE ALL ON FUNCTION public.set_review_group_visibility(uuid,uuid,boolean,boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.set_review_group_visibility(uuid,uuid,boolean,boolean) TO authenticated;
