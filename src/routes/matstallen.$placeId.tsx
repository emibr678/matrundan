...
function summarizeParticipantNames(names: string[]): string | null {
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm">
                  {rating.overall > 0 ? (
                    <>
                      ...
                      <span className="text-muted-foreground">· {visitCountLabel}</span>
                    </>
                  ) : (
                    <span className="font-medium">{visitCountLabel}</span>
                  )}
                </div>
