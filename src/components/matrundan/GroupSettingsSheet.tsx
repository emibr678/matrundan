            ...
            {view === "about" ? (
              <Card className="rounded-2xl border-border/70 p-0">
                ...
                >
                  ...
                  <Badge variant="outline" className="rounded-full">v{APP_VERSION}</Badge>
                </button>
              </Card>
            ) : null}
    ...
    <section>
      ...
      <Card className="space-y-3 rounded-2xl border-border/70 p-4">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          Delade besök syns alltid i historik, besöksstatus och betyg. Inställningen påverkar bara framtida nivåer och märken.
        </p>
  ...
  return (
    <section>
      ...
      <Card className="rounded-2xl border-border/70 p-4">
        ...
        {archived ? (
          <Button type="button" variant="outline" className="mt-3" disabled={submitting} onClick={() => void reactivate()}>
            <ArchiveRestore className="h-4 w-4" /> Återaktivera gruppen
          </Button>
        ) : (
          <Button type="button" variant="outline" className="mt-3 text-destructive hover:text-destructive" disabled={submitting} onClick={() => setConfirmOpen(true)}>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Historik, besök, ställen, betyg och kommentarer bevaras. Gruppen blir skrivskyddad, nästa stopp rensas och aktiva inbjudningar återkallas. Du kan återaktivera gruppen senare.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function InvitationsSection({ groupId }: { groupId: string }) {
  ...
  function shareLink() {
    if (!lastLink) return;
    const data: ShareData = { title: APP_NAME, text: `Häng med i gruppen på ${APP_NAME}.`, url: lastLink };
    ...
    <section>
      ...
      <Card className="space-y-3 rounded-2xl border-border/70 p-4">
        <div className="space-y-1.5">
          ...
          <Input id="inv-email" type="email" placeholder="vän@example.se" value={email} onChange={(event) => setEmail(event.target.value)} />
          <p className="text-xs text-muted-foreground">Med e-post: bara den adressen kan använda länken.</p>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={() => void createInvite(false)} disabled={creating}>Skapa öppen länk</Button>
          <Button onClick={() => void createInvite(true)} disabled={creating || !email.trim()}><Mail className="mr-1 h-4 w-4" /> Skapa för e-post</Button>
        </div>
        {lastLink ? (
          <div className="rounded-xl border border-border/70 bg-muted/40 p-3 text-sm">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <Button size="sm" variant="outline" onClick={() => void copyLink()}><Copy className="h-4 w-4" /> Kopiera</Button>
              <Button size="sm" variant="outline" onClick={shareLink}><Share2 className="h-4 w-4" /> Dela…</Button>
              {lastEmail ? <Button size="sm" onClick={openMailClient}><Mail className="h-4 w-4" /> Öppna e-post</Button> : null}
            </div>
          </div>
        ) : null}
        <div>
          <div className="mb-1 text-xs font-medium text-muted-foreground">Aktiva och nyligen använda inbjudningar</div>
          {loading ? <div className="text-xs text-muted-foreground">Laddar…</div> : items.length === 0 ? <div className="text-xs text-muted-foreground">Inga inbjudningar än.</div> : (
              ...
              {items.map((item) => (
                <li key={item.id} className="flex items-center gap-2 p-2 text-xs">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-muted-foreground">{stateLabel(item.state)} · går ut {formatDate(item.expires_at)}</div>
                  </div>
                  {item.state === "active" ? <Button size="sm" variant="ghost" disabled={busyId === item.id} onClick={() => void revoke(item.id)}>Återkalla</Button> : null}
                </li>
              ))}
            </ul>
          )}
...
function LeaveGroupSection({ groupId, isOwner, onLeft }: { groupId: string; isOwner: boolean; onLeft: () => Promise<void> }) {
  const [open, setOpen] = React.useState(false);
  const [busy, setBusy] = React.useState(false);
  ...
  return (
    <section>
      ...
      <Card className="rounded-2xl border-border/70 p-4">
        <Button variant="ghost" className="w-full justify-start text-destructive" disabled={isOwner} onClick={() => setOpen(true)}>
          <LogOut className="h-4 w-4" /> Lämna gruppen
        </Button>
        {isOwner ? <p className="mt-2 text-xs text-muted-foreground">Som ägare kan du inte lämna gruppen förrän du har överfört ägarskapet.</p> : null}
      </Card>
      <AlertDialog open={open} onOpenChange={setOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Lämna gruppen?</AlertDialogTitle>
            <AlertDialogDescription>Du förlorar åtkomsten direkt. Dina tidigare besök och betyg finns kvar i gruppen.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={busy}>Avbryt</AlertDialogCancel>
            <AlertDialogAction onClick={() => void confirmLeave()} disabled={busy}>{busy ? "Lämnar…" : "Lämna"}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </section>
...
function stateLabel(state: InvitationListItem["state"]): string {
  switch (state) {
    ...
    case "accepted": return "Använd";
    case "expired": return "Utgången";
    case "revoked": return "Återkallad";
  }
}
