...
test("redigerar äldre helhetsbetyg utan modellbyte", async ({ page }) => {
  ...
  const historicalReview = reviewSection.locator(
    '[data-review-id="review-v8-alex"]',
  );

  await expect(
    historicalReview.locator("[data-review-rating]"),
  ).toContainText("4,0");
  ...
  await expect(
    editDialog.getByRole("group", { name: "Detaljbetyg" }),
  ).toHaveCount(0);
  await expect(
  ...
  await editDialog
    .getByRole("button", { name: "Betyg: 3 av 5" })
    .click();
  ...
  await expect(
    reopened.getByRole("group", { name: "Detaljbetyg" }),
  ).toHaveCount(0);
  await expect(
    reopened.getByRole("button", { name: "Lägg till Atmosfär" }),
  ).toHaveCount(0);
});
