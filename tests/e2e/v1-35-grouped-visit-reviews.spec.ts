...
test("exempelgruppen samlar 3+ deltagaromdömen och låter Alex komplettera samma besök", async ({
  ...
  await expect(
    page.getByText("Exempelgrupp · Stockholm", { exact: true }),
  ).toBeVisible();
  await page.goto("/matstallen/p3?visit=v2");

  const visitDialog = page.getByRole("dialog").first();
  await expect(
    visitDialog.getByRole("heading", { name: "Tacoateljén" }),
  ).toBeVisible();
  ...
  await expect(
    reviewSection
      .locator('[data-review-id="review-v2-kim"]')
      .getByText("Kim", { exact: true }),
  ).toBeVisible();
  await expect(
    reviewSection
      .locator('[data-review-id="review-v2-noor"]')
      .getByText("Noor", { exact: true }),
  ).toBeVisible();
  await expect(
    reviewSection.getByText(
      "Den här dolda kommentaren får inte visas i exempelgruppen.",
    ),
  ).toHaveCount(0);
  await expect(
    visitDialog.getByText("Kommentar från gänget", { exact: true }),
  ).toHaveCount(0);
  await expect(
    visitDialog.getByText("Din synlighet", { exact: true }),
  ).toHaveCount(0);
  await expect(
    reviewSection.getByRole("button", { name: "Lägg till ditt omdöme" }),
  ).toHaveCount(1);
  ...
  await reviewSection
    .getByRole("button", { name: "Lägg till ditt omdöme" })
    .click({ force: true });
  const reviewDialog = page.getByRole("dialog").last();
  await expect(
    reviewDialog.getByRole("heading", { name: "Ditt omdöme" }),
  ).toBeVisible();
  await expect(
    reviewDialog.getByText(/Helhetsbetyget räknas automatiskt/),
  ).toBeVisible();
  await expect(reviewDialog.getByText(/komprimeras/i)).toHaveCount(0);
  for (const dimension of ["Smak", "Service", "Prisvärdhet", "Atmosfär"]) {
    await reviewDialog
      .getByRole("button", { name: `${dimension}: 5 av 5` })
      .click();
  }
  await reviewDialog
    .getByLabel("Kommentar (frivilligt)")
    .fill("Mitt eget minne från kvällen.");
  await expect(
    reviewDialog.getByRole("button", { name: "Lägg till bild" }),
  ).toBeVisible();
  ...
  await expect(
    reviewDialog.getByText("Ny bild vald", { exact: true }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Ditt omdöme och din bild är tillagda."),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    reviewSection.getByText("Mitt eget minne från kvällen."),
  ).toBeVisible();
  ...
  await expect(
    reviewSection.getByText("Redigera omdöme", { exact: true }),
  ).toBeVisible();

  await expect(
    visitDialog.getByRole("heading", { name: "Bild från besöket" }),
  ).toBeVisible();
  ...
  await expect(
    editReviewDialog.getByAltText("Din bild från besöket"),
  ).toBeVisible();
  await expect(
    editReviewDialog.getByText("Din nuvarande bild", { exact: true }),
  ).toBeVisible();
  await expectNoLocatorOverflow(
    editReviewDialog,
    "Redigera omdöme med befintlig bild",
  );
  ...
  await expect(
    editReviewDialog.getByRole("button", { name: "Ångra" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Ditt omdöme är uppdaterat och bilden borttagen."),
  ).toBeVisible();
  ...
  await expectNoLocatorOverflow(
    visitDialog,
    "kompletterat fleromdömesscenario",
  );
});
...
test("historiskt 3D-omdöme är komplett och Atmosfär läggs till först vid sparning", async ({
  ...
  const visitDialog = page.getByRole("dialog").first();
  const reviewSection = visitDialog.getByLabel("Gängets omdömen");
  const historicalReview = reviewSection.locator(
    '[data-review-id="review-v10-alex"]',
  );

  await expect(historicalReview.locator("[data-review-rating]")).toContainText(
    "4,7",
  );
  await expect(
    historicalReview.getByText("Atmosfär", { exact: true }),
  ).toHaveCount(0);

  await historicalReview
    .getByRole("button", { name: "Redigera omdöme" })
    .click();
  let editDialog = page.getByRole("dialog", { name: "Redigera ditt omdöme" });

  await expect(
    editDialog.getByText("Äldre detaljbetyg (frivilligt)"),
  ).toHaveCount(0);
  await expect(
    editDialog.getByRole("button", { name: /Atmosfär:/ }),
  ).toHaveCount(0);
  ...
  await expect(
    scoreFields.getByText("Ingick inte i betyget när omdömet skapades."),
  ).toBeVisible();
  await expect(
    scoreFields.getByRole("button", { name: "Lägg till Atmosfär" }),
  ).toBeVisible();
  ...
  await expect(
    editDialog.getByRole("button", { name: "Atmosfär: 3 av 5" }),
  ).toBeVisible();
  await expect(
  ...
  await expectNoHorizontalOverflow(
    page,
    "frivillig Atmosfär-komplettering på 360 px",
  );
  ...
  await expect(
    editDialog.getByRole("button", { name: "Lägg till Atmosfär" }),
  ).toHaveCount(0);
  await expect(
    editDialog.getByRole("button", { name: "Atmosfär: 3 av 5" }),
  ).toBeVisible();
