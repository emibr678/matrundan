...
async function expectNoHorizontalOverflow(
  ...
  expect(
    ...
    `${context}: sidan ska inte overflowa`,
...
test("Reaktionsväljaren är förankrad, fokusstyrd och flyttar inte nästa omdöme", async ({
  page,
}) => {
  ...
  const reactionBar = samReview.locator(
    '[data-review-reactions="review-v2-sam"]',
  );
  await expect(samReview).toBeVisible();
  await expect(
    samReview.getByText("Bra tempo och generösa portioner."),
  ).toBeVisible();
  ...
  const ownReactionChip = samReview.getByRole("button", {
    name: /Ser gott ut: 1 reaktion/,
  });
  ...
  await page
    .getByRole("button", { name: "Ta bort din ser gott ut-reaktion" })
    .click();
  await expect(
    samReview.getByRole("button", { name: /Ser gott ut:/ }),
  ).toHaveCount(0);

  await expect(samReview.getByRole("button", { name: /Roligt:/ })).toHaveCount(
    0,
  );
  await expect(
    samReview.getByRole("button", { name: /Visa detaljer/ }),
  ).toHaveCount(0);
  ...
  await expect(
    dialog.getByText("Gruppens betyg", { exact: true }),
  ).toBeVisible();
  ...
  const detailTops = await details
    .locator(":scope > div")
    .evaluateAll((items) =>
  ...
  await expect(page.getByText("Alex (Du)", { exact: true })).toBeVisible();
  await page
    .getByRole("button", { name: "Ta bort din hjärta-reaktion" })
    .click();
  ...
  await updatedSamReview
    .getByRole("button", { name: "Lägg till reaktion på Sams omdöme" })
    .click();
  picker = page.getByRole("group", { name: "Välj reaktion" });
  ...
  await updatedSamReview
    ...
    .click();
  await expect(
    updatedSamReview.getByRole("button", { name: /Roligt:/ }),
  ).toHaveCount(0);
  ...
  await expect(
    page
      ...
      .getByText("Robin", { exact: true }),
  ).toBeVisible();

  await expectNoHorizontalOverflow(
    page,
    reactionBar,
    "reaktionsflöde på 360 px",
  );
  await expectNoHorizontalOverflow(page, dialog, "omdömeskort på 360 px");
});

test("Hem fokuserar senaste omdömet tills användaren interagerar", async ({
  page,
}) => {
  await page.goto("/exempel");
  await expect(
  ...
  await expect(
    latestReviewLink.getByText("Robin", { exact: true }),
  ).toBeVisible();
  await expect(
  ...
  await expect(focusedReview).toHaveAttribute(
    "data-review-highlighted",
    "true",
  );
  ...
  await expect(focusedReactButton).not.toHaveAttribute(
    "data-emphasized",
    "true",
  );

  const ownReview = dialog
  ...
  await expect(
    ownReview.getByRole("button", { name: "Redigera omdöme" }),
  ).toBeVisible();
  ...
  const scrollTopBeforeReaction = await dialog.evaluate(
    (element) => element.scrollTop,
  );
  await focusedReactButton.click();
  await expect(focusedReview).not.toHaveAttribute(
    "data-review-highlighted",
    "true",
  );
  await page
    ...
    .click();
  await expect(
    focusedReview.getByRole("button", { name: /Hjärta: 1 reaktion/ }),
  ).toBeVisible();
  await expect
    .poll(async () => {
      const scrollTopAfterReaction = await dialog.evaluate(
        (element) => element.scrollTop,
      );
...
test("deep-linkat omdöme öppnas synligt i samma besöksdetalj", async ({
  page,
}) => {
  await page.goto("/exempel");
  await expect(
  ...
  const reactButton = focusedReview.getByRole("button", {
    name: "Lägg till reaktion på Sams omdöme",
  });
  ...
  await expect(
    focusedReview.getByRole("button", { name: /Visa detaljer/ }),
  ).toHaveCount(0);
  await expectNoHorizontalOverflow(
    page,
    dialog,
    "deep-linkat omdöme på 360 px",
  );
...
test("reaktionsraden och betygshierarkin förblir kompakta på desktop", async ({
  page,
}) => {
  ...
  await expect(heartChip).toContainText("Robin");
  await expect(
    samReview.getByRole("button", {
      name: "Lägg till reaktion på Sams omdöme",
    }),
  ).toBeVisible();
  await expect(
    samReview.getByRole("button", { name: /Fler reaktioner/ }),
  ).toHaveCount(0);
  await expect(
    samReview.getByRole("button", { name: /Visa detaljer/ }),
  ).toHaveCount(0);
