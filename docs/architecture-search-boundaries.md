...
Vid val av en boundary-kandidat verifieras först om kandidatens eget Geoapify-`place_id` ger Polygon/MultiPolygon via Place Details. `details` är förstahandsval och `details.full_geometry` används bara som tillägg när standarddetaljerna saknar polygon. Geoapifys autocomplete kan dock representera ett administrativt namn med en punkt/node även när en separat administrativ boundary finns. Om direktgeometrin därför fortfarande saknas används Boundaries API `part-of` som riktad fallback med administrativ boundary och `geometry_1000`. Fallbacken får endast acceptera en Polygon/MultiPolygon vars namn motsvarar användarens valda områdesetikett; orten `Nacka` får exempelvis inte automatiskt bli `Nacka kommun`.
