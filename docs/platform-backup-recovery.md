...
Restoreövning ska göras mot ett separat tomt testprojekt, aldrig mot staging eller production. Normal ordning är:

1. verifiera `backup-manifest.json` innan någon import;
2. skapa ett tomt Supabase-testprojekt;
3. applicera repoets aktuella migrationer i ordning;
...
6. återetablera environment-specifika Auth-providerinställningar, callbacks och secrets från driftkonfiguration – aldrig från backupfilerna;
...
- återställa den till ett tomt separat projekt;
